...
<div class="space-y-6 sticky top-24">
    ...
    <div class="relative w-full rounded-2xl overflow-hidden shadow-soft border border-slate-200/80 bg-slate-900 group" id="sidebarSliderWidget">
        
        <!-- Slider Track Container with Smooth Height Transition -->
        <div class="relative w-full overflow-hidden transition-[height] duration-500 ease-in-out min-h-[140px]" id="sidebarSliderContainer" style="transition: height 0.45s cubic-bezier(0.4, 0, 0.2, 1);">
            @foreach($sliderItems as $idx => $slide)
                ...
                @endphp
                <div class="sidebar-slide absolute inset-0 w-full transition-opacity duration-500 ease-in-out {{ $idx === 0 ? 'opacity-100 z-10' : 'opacity-0 z-0 pointer-events-none' }}" data-index="{{ $idx }}">
                    <a href="{{ $slideUrl }}" class="block w-full relative group/img">
                        <!-- Image Element -->
                        <img src="{{ $slideImg }}" alt="{{ $slideTitle }}" class="w-full h-auto object-cover block group-hover/img:scale-105 transition-transform duration-500">
                        
                        <!-- Gradient Overlay with Text if slide title exists -->
                        @if(!empty($slideTitle))
                        <div class="absolute inset-0 bg-gradient-to-t from-black/85 via-black/20 to-transparent flex flex-col justify-end p-4 pointer-events-none">
                            <span class="self-start px-2 py-0.5 rounded-md text-[10px] font-bold bg-white/90 backdrop-blur-md text-slate-900 shadow mb-1">
                                {{ $slideType }}
                            </span>
        ...
        <button type="button" onclick="prevSidebarSlide(event)" class="absolute left-2.5 top-1/2 -translate-y-1/2 z-20 w-8 h-8 rounded-full bg-slate-900/60 hover:bg-slate-900 text-white flex items-center justify-center text-xs backdrop-blur-md border border-white/20 transition opacity-80 hover:opacity-100 shadow-md transform active:scale-95" aria-label="Slide Sebelumnya">
            <i class="fa-solid fa-chevron-left"></i>
        </button>

        <button type="button" onclick="nextSidebarSlide(event)" class="absolute right-2.5 top-1/2 -translate-y-1/2 z-20 w-8 h-8 rounded-full bg-slate-900/60 hover:bg-slate-900 text-white flex items-center justify-center text-xs backdrop-blur-md border border-white/20 transition opacity-80 hover:opacity-100 shadow-md transform active:scale-95" aria-label="Slide Berikutnya">
        ...
        <div class="absolute bottom-3 left-1/2 -translate-x-1/2 z-20 flex items-center gap-1.5" id="sidebarSliderDots">
            @foreach($sliderItems as $idx => $slide)
                <button type="button" onclick="goToSidebarSlide({{ $idx }})" class="sidebar-dot h-1.5 rounded-full transition-all duration-300 {{ $idx === 0 ? 'w-5 bg-brand-400' : 'w-1.5 bg-white/60 hover:bg-white' }}" aria-label="Slide {{ $idx + 1 }}"></button>
            @endforeach
        </div>
        @endif
    ...
    <div class="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-soft">
        ...
        <div class="space-y-3">
            @foreach($sidePengumuman as $peng)
                <a href="{{ url($peng->url) }}" class="block p-2.5 rounded-xl hover:bg-slate-50 border border-transparent hover:border-slate-100 transition group">
                    ...
                    <h5 class="text-xs font-bold text-slate-800 group-hover:text-brand-600 transition-colors line-clamp-2 leading-snug">
                        {{ $peng->title }}
                    </h5>
                </a>
            @endforeach
    ...
    <div class="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-soft">
        <h4 class="text-xs font-extrabold text-slate-900 uppercase tracking-wider pb-3 border-b border-slate-100 mb-3 flex items-center gap-2">
                ...
                <a href="{{ url($pop->url) }}" class="flex items-center gap-3 group">
                    <div class="w-14 h-14 rounded-xl overflow-hidden bg-slate-100 flex-shrink-0">
                        @if(!empty($pop->thumbnail))
                            <img src="{{ $pop->thumbnail }}" alt="{{ $pop->title }}" class="w-full h-full object-cover group-hover:scale-110 transition duration-300">
                        @else
    ...
    <div class="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-soft">
        ...
        <div class="space-y-1.5">
            @foreach($sideCategories as $cat)
                <a href="{{ url($cat->url) }}" class="flex items-center justify-between p-2 rounded-xl hover:bg-brand-50 text-xs font-semibold text-slate-700 hover:text-brand-600 transition">
    ...
    <div class="bg-white rounded-2xl p-5 border border-slate-200/80 shadow-soft">
        ...
        <div class="space-y-2.5">
            @foreach($sideDocs as $doc)
                <a href="{{ url($doc->url) }}" class="flex items-center justify-between p-2.5 rounded-xl bg-slate-50 hover:bg-purple-50 hover:text-purple-700 transition group border border-slate-100">
                    <div class="flex items-center gap-2.5 pr-2">
                        <i class="fa-solid fa-file-pdf text-purple-500 text-sm"></i>
                        <span class="text-xs font-bold text-slate-800 group-hover:text-purple-700 line-clamp-1">{{ $doc->title }}</span>
                    </div>
    ...
    <div class="rounded-2xl p-5 brand-gradient text-white shadow-card space-y-3">
        ...
        <a href="https://wa.me/{{ preg_replace('/[^0-9]/', '', get_option('nomor_whatsapp') ?? '628123456789') }}" target="_blank" class="w-full inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl font-bold text-xs bg-white text-slate-900 hover:bg-slate-100 shadow transition">
