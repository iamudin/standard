@php
    $categories = query()->index_category('pengumuman');
@endphp
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 w-full">
    ...
    <nav class="flex items-center flex-wrap gap-2 text-xs font-semibold text-slate-500 mb-6">
        ...
        <span class="text-slate-900">{{ $module->title ?? 'Pengumuman Resmi' }}</span>
    </nav>
    ...
    <div class="rounded-3xl hero-gradient text-white p-6 sm:p-10 mb-8 sm:mb-10 shadow-card relative overflow-hidden">
        ...
        <div class="relative z-10 space-y-2">
            <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-white/10 text-xs font-bold text-amber-200 backdrop-blur-md">
            ...
            <h1 class="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-white tracking-tight break-words">
                {{ $module->title ?? 'Pengumuman Resmi' }}
            </h1>
            <p class="text-xs sm:text-sm text-slate-200 max-w-2xl leading-relaxed">
                {{ $module->description ?? 'Informasi penting mengenai kebijakan, agenda, jadwal pelayanan, dan pengumuman kedinasan resmi.' }}
            </p>
        </div>
    </div>
    ...
    @if($categories->count() > 0)
        <div class="flex items-center gap-2 overflow-x-auto pb-3 mb-8 no-scrollbar w-full">
            <a href="{{ url('pengumuman') }}" class="px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap bg-brand-600 text-white shadow-sm flex-shrink-0">
                Semua Kategori
            </a>
            @foreach($categories as $c)
                <a href="{{ url($c->url) }}" class="px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap bg-white text-slate-700 hover:bg-slate-100 border border-slate-200 transition flex-shrink-0">
        ...
        <div class="lg:col-span-8 min-w-0 space-y-4">
                            ...
                            <div class="flex flex-wrap items-center justify-between gap-2 mb-3">
                                <div class="flex items-center gap-2">
                                    <span class="px-2.5 py-1 rounded-lg text-[11px] font-bold bg-amber-50 text-amber-700 border border-amber-200/60 flex items-center gap-1">
                                        <i class="fa-solid fa-bullhorn text-[10px]"></i>
                                        <span>{{ $row->category->name ?? 'Pengumuman' }}</span>
                                    </span>
                                </div>
            ...
            @else
                <div class="bg-white rounded-3xl p-8 sm:p-12 text-center border border-slate-200 shadow-soft">
                    ...
                    <h3 class="text-base font-bold text-slate-900 mb-1">Belum Ada Pengumuman</h3>
