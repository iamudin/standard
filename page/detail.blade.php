<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <nav class="flex items-center gap-2 text-xs font-semibold text-slate-500 mb-6">
        ...
        <span class="text-slate-900">{{ $detail->title ?? 'Halaman Informasi' }}</span>
    </nav>
        ...
        <div class="lg:col-span-8">
            <article class="bg-white rounded-3xl p-6 sm:p-10 border border-slate-200/80 shadow-soft space-y-6">
                ...
                <div class="space-y-3 pb-6 border-b border-slate-100">
                    <span class="inline-flex items-center gap-1.5 px-3 py-1 rounded-full bg-slate-100 text-xs font-bold text-slate-700">
                    ...
                    <div class="flex items-center gap-3 text-xs text-slate-400 font-medium">
                        <span><i class="fa-regular fa-calendar mr-1"></i> Terakhir Diperbarui: {{ $detail->updated_at?->translatedFormat('d F Y') ?? $detail->created ?? date('d M Y') }}</span>
                    </div>
                </div>
                ...
                <div class="article-body prose prose-slate max-w-none text-slate-700 text-sm sm:text-base leading-relaxed space-y-4">
                    @if(!empty($detail->content))
                        {!! $detail->content !!}
                    @else
                        <p class="text-slate-500 italic">Tidak ada rincian konten pada halaman ini.</p>
                    @endif
                </div>
