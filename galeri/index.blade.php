@php
    $categories = function_exists('query') ? query()->index_sort_by_category('galeri') : collect();
@endphp
...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <nav class="flex items-center gap-2 text-xs font-semibold text-slate-500 mb-6">
        ...
        <span class="text-slate-900">{{ $module->title ?? 'Galeri Foto' }}</span>
    </nav>
    ...
    <div class="rounded-3xl hero-gradient text-white p-8 sm:p-10 mb-10 shadow-card relative overflow-hidden">
        ...
        <div class="relative z-10 space-y-2">
            ...
            <h1 class="text-2xl sm:text-3xl lg:text-4xl font-extrabold text-white tracking-tight">
                {{ $module->title ?? 'Galeri Foto & Dokumentasi' }}
            </h1>
            <p class="text-xs sm:text-sm text-slate-200 max-w-2xl leading-relaxed">
                {{ $module->description ?? 'Koleksi album foto kegiatan, sarana prasarana, fasilitas publik, dan dokumentasi momen penting.' }}
            </p>
        </div>
    </div>
    ...
    @if($categories->count() > 0)
        <div class="flex items-center gap-2 overflow-x-auto pb-4 mb-8 no-scrollbar">
            <a href="{{ url('galeri') }}" class="px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap bg-brand-600 text-white shadow-sm">
                Semua Kategori
            </a>
        ...
        <div class="lg:col-span-8 space-y-8">
            ...
            @else
                <div class="bg-white rounded-3xl p-12 text-center border border-slate-200 shadow-soft">
                    ...
                    <h3 class="text-base font-bold text-slate-900 mb-1">Belum Ada Galeri</h3>
                    <p class="text-xs text-slate-500 max-w-sm mx-auto mb-6">
                        Saat ini belum ada album galeri foto yang dipublikasikan.
                    </p>
