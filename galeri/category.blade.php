...
<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
    ...
    <div class="rounded-3xl hero-gradient text-white p-8 sm:p-10 mb-10 shadow-card relative overflow-hidden">
        ...
        <div class="relative z-10 space-y-2">
            <div class="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-white/10 text-xs font-bold text-brand-200 backdrop-blur-md">
                ...
                <span>Kategori Galeri Foto</span>
            </div>
            ...
            <p class="text-xs sm:text-sm text-slate-200 max-w-2xl leading-relaxed">
                {{ $category->description ?? 'Daftar album foto dan dokumentasi kegiatan pada kategori ' . $category->name . '.' }}
            </p>
        </div>
    </div>
    ...
    @if($siblingCategories->count() > 0)
        <div class="flex items-center gap-2 overflow-x-auto pb-4 mb-8 no-scrollbar">
            <a href="{{ url('galeri') }}" class="px-4 py-2 rounded-xl text-xs font-bold whitespace-nowrap bg-white text-slate-700 hover:bg-slate-100 border border-slate-200 transition">
                Semua Kategori
            </a>
        ...
        <div class="lg:col-span-8 space-y-8">
            ...
            @else
                <div class="bg-white rounded-3xl p-12 text-center border border-slate-200 shadow-soft">
                    ...
                    <h3 class="text-base font-bold text-slate-900 mb-1">Belum Ada Galeri di Kategori Ini</h3>
                    <p class="text-xs text-slate-500 max-w-sm mx-auto mb-6">
                        Belum ada album foto kegiatan yang dipublikasikan pada kategori <strong>{{ $category->name }}</strong>.
                    </p>
                    <a href="{{ url('galeri') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl font-bold text-xs text-white brand-gradient shadow">
